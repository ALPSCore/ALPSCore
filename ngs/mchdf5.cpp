...
namespace alps {
    ...
    namespace detail {
        ...
        hid_t open_attribute(mchdf5 const & ar, hid_t file_id, std::string path) {
    ...
    #define ALPS_NGS_MCHDF5_IS_DATATYPE(T)                                                                                                                         \
        template<> bool mchdf5::is_datatype< T >(std::string path) const {                                                                                         \
            try {                                                                                                                                                  \
                hid_t type_id;                                                                                                                                     \
    ...
    std::string const & mchdf5::get_filename() const {
        return context_->filename_;
    }

    std::string mchdf5::encode_segment(std::string segment) const {
        char chars[] = {'&', '/'};
    ...
    std::string mchdf5::decode_segment(std::string segment) const {
    ...
    std::string mchdf5::get_context() const {
        return current_;
    }

    void mchdf5::set_context(std::string const & context) {
        current_ = context;
    }

    std::string mchdf5::complete_path(std::string path) const {
        if (path.size() && path[0] == '/')
            return path;
    ...
    bool mchdf5::is_data(std::string path) const {
           try {
    ...
    bool mchdf5::is_scalar(std::string path) const {
        try {
            hid_t space_id;
            path = complete_path(path);
            if (path.find_last_of('@') != std::string::npos && is_attribute(path)) {
    ...
    bool mchdf5::is_string(std::string path) const {
        try {
            hid_t type_id;
            path = complete_path(path);
    ...
    bool mchdf5::is_null(std::string path) const {
        try {
            hid_t space_id;
            path = complete_path(path);
            if (path.find_last_of('@') != std::string::npos) {
    ...
    bool mchdf5::is_complex(std::string path) const {
        if (is_datatype<double>(path)) {
            if (path.find_last_of('@') != std::string::npos)
                return is_scalar(path.substr(0, path.find_last_of('@') - 1) + "@__complex__:" + path.substr(path.find_last_of('@') + 1));
            else 
                return is_scalar(path + "@__complex__");
        } else
            return false;
    }

    std::vector<std::string> mchdf5::list_children(std::string path) const {
        try {
            if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
    ...
    std::vector<std::string> mchdf5::list_attributes(std::string path) const {
        try {
            if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
    ...
    std::vector<std::size_t> mchdf5::extent(std::string path) const {
        try {
            path = complete_path(path);
            if (is_null(path))
    ...
    std::size_t mchdf5::dimensions(std::string path) const {
        try {
            path = complete_path(path);
            if (path.find_last_of('@') != std::string::npos) {
                detail::attribute_type attr_id(detail::open_attribute(*this, context_->file_id_, path));
    ...
    void mchdf5::create_group(std::string path) const {
        try {
            if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
                ALPS_NGS_THROW_RUNTIME_ERROR("no group path: " + path)
            if (is_data(path))
    ...
    void mchdf5::delete_data(std::string path) const {
        try {
            if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
                ALPS_NGS_THROW_RUNTIME_ERROR("no data path: " + path)
            if (is_data(path))
    ...
    void mchdf5::delete_group(std::string path) const  {
        try {
            if ((path = complete_path(path)).find_last_of('@') != std::string::npos)
                ALPS_NGS_THROW_RUNTIME_ERROR("no group path: " + path)
            if (is_group(path))
    ...
    void mchdf5::delete_attribute(std::string path) const {
        try {
            if ((path = complete_path(path)).find_last_of('@') == std::string::npos)
                ALPS_NGS_THROW_RUNTIME_ERROR("no attribute path: " + path)
            // TODO: implement
    ...
    void mchdf5::set_complex(std::string path) {
        if (path.find_last_of('@') != std::string::npos)
            write(path.substr(0, path.find_last_of('@') - 1) + "@__complex__:" + path.substr(path.find_last_of('@') + 1), true);
        else
    ...
    #define ALPS_NGS_MCHDF5_READ_SCALAR(T)                                                                                                                         \
        void mchdf5::read(std::string path, T & value) const {                                                                                                     \
            try {                                                                                                                                                  \
                if ((path = complete_path(path)).find_last_of('@') == std::string::npos) {                                                                         \
    ...
    #define ALPS_NGS_MCHDF5_READ_VECTOR(T)                                                                                                                         \
        void mchdf5::read(std::string path, T * value, std::vector<std::size_t> chunk, std::vector<std::size_t> offset) const {                                    \
            try {                                                                                                                                                  \
    ...
    #define ALPS_NGS_MCHDF5_WRITE_SCALAR(T)                                                                                                                        \
        void mchdf5::write(std::string path, T value) const {                                                                                                      \
            try {                                                                                                                                                  \
                if (!context_->write_)                                                                                                                             \
                ...
                if ((path = complete_path(path)).find_last_of('@') == std::string::npos) {                                                                         \
                    ...
                    detail::check_error(H5Dwrite(data_id, type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, &value));                                                        \
                    detail::check_data(data_id);                                                                                                                   \
                } else {                                                                                                                                           \
                    hid_t parent_id;                                                                                                                               \
                    if (is_group(path.substr(0, path.find_last_of('@') - 1)))                                                                                      \
                    ...
                    detail::check_error(H5Awrite(data_id, type_id, &value));                                                                                       \
                ...
                if ((path = complete_path(path)).find_last_of('@') == std::string::npos) {                                                                         \
                    ...
                    if (std::accumulate(size.begin(), size.end(), std::size_t(0)) == 0) {                                                                          \
                        ...
                        if (std::equal(chunk.begin(), chunk.end(), size.begin()))                                                                                  \
                            detail::check_error(H5Dwrite(raii_id, type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, value));                                                 \
                        else {                                                                                                                                     \
                            ...
                            detail::check_error(H5Dwrite(raii_id, type_id, mem_id, space_id, H5P_DEFAULT, value));                                                 \
                        }                                                                                                                                          \
                    }                                                                                                                                              \
                } else {                                                                                                                                           \
                    hid_t parent_id;                                                                                                                               \
                    ...
                    if (std::accumulate(size.begin(), size.end(), std::size_t(0)) == 0) {                                                                          \
                        ...
                        {                                                                                                                                          \
                            ...
                            if (std::equal(chunk.begin(), chunk.end(), size.begin()))                                                                              \
                                detail::check_error(H5Awrite(raii_id, type_id, value));                                                                            \
                            else                                                                                                                                   \
                                ALPS_NGS_THROW_RUNTIME_ERROR("Not Implemented, path: " + path)                                                                     \
                        }                                                                                                                                          \
                    }                                                                                                                                              \
    ...
    #define ALPS_NGS_MCHDF5_IMPLEMENT_FREE_FUNCTIONS(T)                                                                                                            \
        namespace detail {                                                                                                                                         \
            alps::scalar_type< T >::type * get_pointer< T >::apply( T & value) {                                                                                   \
                return &value;                                                                                                                                     \
            }                                                                                                                                                      \
                                                                                                                                                                   \
            alps::scalar_type< T >::type const * get_pointer< T const >::apply( T const & value) {                                                                 \
                return &value;                                                                                                                                     \
            }                                                                                                                                                      \
                                                                                                                                                                   \
    ...
    std::string mchdf5::file_key(std::string filename, bool write, bool compress) const {
        return std::string(write ? "w" : "r") + (compress ? "c" : "_") + "@" + filename;
    }

    std::map<std::string, std::pair<detail::mccontext *, std::size_t> > mchdf5::ref_cnt_;
