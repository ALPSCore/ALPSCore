...
namespace alps {
    ...
    */
        ...
        >::type operator<< (archive & ar, detail::make_pvp_proxy<T> const & proxy) {
            try {
				/* TODO: fixit!
				std::cerr << "complex path: " << proxy.path_ << std::endl;
				*/
        ...
        >::type operator<< (archive & ar, detail::make_pvp_proxy<T> const & proxy) {
            try {
				/* TODO: fixit!
				if (proxy.path_ == "/cplx") {
					std::cerr << "none complex path: " << std::endl;
					
					
					std::cerr << (boost::is_same<typename boost::remove_const<T>::type, std::complex<double> >::value ? "true" : "false")
					  << " " << typeid(typename boost::remove_const<T>::type).name() << " " << (has_complex_elements<typename boost::remove_const<T>::type>::value ? "true" : "false") << " " << proxy.path_ << std::endl;
					 
					size_t len; 
					int s; 
					char* p = abi::__cxa_demangle(typeid(T).name(), 0, &len, &s);
					std::cerr << typeid(T).name() << ": " << p << std::endl;
					char* p2 = abi::__cxa_demangle(typeid(typename boost::remove_const<T>::type).name(), 0, &len, &s);
					std::cerr << typeid(typename boost::remove_const<T>::type).name() << ": " << p2 << std::endl;
					std::cerr << "boost::is_same<std::complex<double>, std::complex<double> >::value " << (boost::is_same<std::complex<double>, std::complex<double> >::value ? "true" : "false") << std::endl;
					std::cerr << "boost::is_same<T, std::complex<double> >::value " << (boost::is_same<T, std::complex<double> >::value ? "true" : "false") << std::endl;
					std::cerr << "typeid(boost::is_same<T, std::complex<double> >) " << typeid(typename boost::is_same<T, std::complex<double> >::type).name() << std::endl;
					std::cerr << "boost::is_same<typename boost::remove_const<T>::type, std::complex<double> >::value " << (boost::is_same<typename boost::remove_const<T>::type, std::complex<double> >::value ? "true" : "false") << std::endl;
					std::cerr << "same typeid:" << (typeid(typename boost::remove_const<T>::type) == typeid(std::complex<double>) ? "true" : "false") << std::endl;
				}
				*/
                save(ar, proxy.path_, proxy.value_);
            } catch (std::exception & ex) {
