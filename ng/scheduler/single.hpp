...
#ifndef ALPS_NG_SCHEDULER_SINGLE_HPP
#define ALPS_NG_SCHEDULER_SINGLE_HPP
        ...
        namespace scheduler {
            ...
            class single {
                public:
                    ...
                    typedef ::alps::ng::result_set results_type;
                    typedef std::vector<std::string> result_names_type;

                    single(parameters_type const & p, std::size_t seed_offset = 0)
                        : params(p)
                        , next_check(8)
...
/*                        for(result_names_type::const_iterator it = names.begin(); it != names.end(); ++it)
                            if (dynamic_cast<AbstractSimpleObservable<double> const *>(&results[*it]) != NULL)
                                partial_results[*it] = ::boost::make_shared<observables::base>(new ::alps::ng::observables::evaluator<double>(dynamic_cast<AbstractSimpleObservable<double> const &>(results[*it])));
                            else if (dynamic_cast<AbstractSimpleObservable<std::valarray<double> > const *>(&results[*it]) != NULL)
                                partial_results[*it] = ::boost::make_shared<observables::base>(new observables::evaluator<std::vector<double> >(dynamic_cast<AbstractSimpleObservable<std::valarray<double> > const &>(results[*it])));
                            else
