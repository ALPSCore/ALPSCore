...
namespace alps {
  namespace hdf5 {
    namespace detail {
      ...
      hdf5_context::hdf5_context(std::string const & filename, hid_t file_id, bool compress)
            : _compress(compress)
            , _revision(0)
            , _state_id(-1)
            , _log_id(-1)
    ...
    hid_t archive::get_native_type(std::string) const 
    { 
    ...
    std::map<std::pair<std::string, bool>, boost::weak_ptr<detail::hdf5_context> > archive::_pool;
