...
if [ ".$EIGEN3_INCLUDE_DIR" != . ]; then
    echo "Eigen3 will be searched in $EIGEN3_INCLUDE_DIR directory"
    defs="$defs -DEIGEN3_INCLUDE_DIR=$EIGEN3_INCLUDE_DIR"
else
    echo "Eigen3 will be downloaded"
    defs="$defs -DALPS_INSTALL_EIGEN=yes"
fi
