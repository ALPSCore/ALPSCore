...
* The language of the libraries is C++14 to minimize incompatibilities with existing HPC (High Performance Computing) systems (if you really, really need to use a C++17 or newer feature,  [it can be discussed](https://github.com/ALPSCore/ALPSCore/issues)). The unit tests are compiled as C++17, because the bundled Google Test requires it, so building them (`Testing=ON`, the default) needs a C++17 compiler; with an older compiler, configure with `-DTesting=OFF`.
