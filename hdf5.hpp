	...
	namespace detail {
		...
		template <typename Tag> class archive: boost::noncopyable {
			public:
				...
				template<typename T> void set_value(std::string const & p, T const & v) {
					if (p.find_last_of('@') != std::string::npos) {
						set_attr(compute_path(p.substr(0, p.find_last_of('@') - 1)), p.substr(p.find_last_of('@') + 1), v);
					} else
						set_data(compute_path(p), v);
				}
...
// = = = = = = = = = = set_data = = = = = = = = = =
				...
				template<typename T> typename boost::disable_if<boost::is_scalar<T> >::type set_data(std::string const & p, T const & v) {
					if (!v.size())
						...
						set_data(p, &const_cast<typename T::value_type &>(v[0]), v.size());
				}
					...
					if (v.size() == 0)
						...
						for (std::size_t i = 1; i < v.size(); ++i)
							if (v[i].size() != v[0].size())
								...
								detail::error_type(H5Dwrite(data_id, type_id, mem_id, space_id, H5P_DEFAULT, &(v[1][0])));
							}
					}
				}
...
// = = = = = = = = = = set_attr = = = = = = = = = =
				template<typename T> typename boost::enable_if<boost::is_scalar<T> >::type set_attr(std::string const & p, std::string const & s, T const & v) {
					if (is_group(p))
				...
				template<typename T> void set_attr(std::string const & p, std::string const & s, std::vector<std::valarray<T> > const & v) {
...
// = = = = = = = = = = private = = = = = = = = = =
		...
		template <typename T, pvp_type U = plain> class pvp {
			public:
				...
				void serialize(archive<write> & ar) const { _v.serialize(ar); }
				void set_value(archive<write> & ar) const { ar.set_value(_p, _v); }
				void get_value(archive<read> & ar) const { std::cerr << "Not impl" << std::endl; }
			private:
				std::string _p;
				mutable T _v;
		};
		...
		template <typename T> class pvp<T, array> {
			public:
				...
				std::string get_path() const { return _p; }
				void serialize(archive<write> & ar) const { _v->serialize(ar); }
				void set_value(archive<write> & ar) const { ar.set_data(ar.compute_path(_p), _v, _s); }
				void get_value(archive<read> & ar) const { std::cerr << "Not impl" << std::endl; }
			private:
				std::string _p;
				std::size_t _s;
		...
		template<typename T, typename U, bool B> struct has_serialize_checker {
			enum { value = false };
		};
		template<typename T, typename U> struct has_serialize_checker<T, U, true> {
			enum { value = sizeof(has_serialize_helper<T, U>(NULL)) == sizeof(boost::type_traits::yes_type) };
		};
		template <typename T, typename U> struct has_serialize {
			typedef typename boost::mpl::bool_<
				...
				has_serialize_checker<typename boost::remove_reference<typename boost::remove_cv<T>::type>::type, U, boost::is_class<T>::value>::value
			>::type type;
		};
		template <typename T, pvp_type U> typename boost::enable_if<typename has_serialize<T, write>::type, archive<write> &>::type operator<< (archive<write> & ar, pvp<T, U> const & v) {
		...
		template <typename T, pvp_type U> typename boost::disable_if<typename has_serialize<T, write>::type, archive<write> &>::type operator<< (archive<write> & ar, pvp<T, U> const & v) {
			v.set_value(ar);
			return ar;
		}
		template <typename T, pvp_type U> typename boost::enable_if<typename has_serialize<T, read>::type, archive<read> &>::type operator>> (archive<read> & ar, pvp<T, U> const & v) {
			std::string c = ar.get_context();
			ar.set_context(ar.compute_path(v.get_path()));
			v.serialize(ar);
			ar.set_context(c);
		...
		template <typename T, pvp_type U> typename boost::disable_if<typename has_serialize<T, read>::type, archive<read> &>::type operator>> (archive<read> & ar, pvp<T, U> const & v) {
			v.get_value(ar);
			return ar;
		}
	}	
	...
	typedef detail::serializable serializable;
}
