...
// = = = = = = = = = = set_data = = = = = = = = = =
			...
			template<typename T> typename boost::disable_if<boost::is_scalar<T> >::type set_data(std::string const & p, T const & v) {
				if (v.size() == 0)
					set_data(p, static_cast<typename T::value_type const *>(NULL), NULL);
				else
...
// = = = = = = = = = = set_attr = = = = = = = = = =			
			...
			template<typename T> typename boost::disable_if<boost::is_scalar<T> >::type set_attr(std::string const &, std::string const &, T const &) {
...
// = = = = = = = = = = private = = = = = = = = = =			
			...
			public:
				...
				void set_value(hdf5::archive<hdf5::write> & ar) const { 
					if (_p.find_last_of('@') != std::string::npos) {
						ar.set_attr(ar.compute_path(_p.substr(0, _p.find_last_of('@') - 1)), _p.substr(_p.find_last_of('@') + 1), _v);
					} else
						ar.set_data(ar.compute_path(_p), _v);
				}
				void get_value(hdf5::archive<hdf5::read> & ar) const { std::cerr << "Not impl" << std::endl; }
			private:
				std::string _p;
				mutable T _v;
		};
	...
	>, oarchive &>::type operator<< (oarchive & ar, detail::pvp<T, U> const & v) {
	...
	>, iarchive &>::type operator>> (iarchive & ar, detail::pvp<T, U> const & v) {
		std::string c = ar.get_context();
		ar.set_context(ar.compute_path(v.get_path()));
		v.serialize(ar);
		ar.set_context(c);
